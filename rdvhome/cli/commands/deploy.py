...
class Command(SimpleCommand):
    ...
    def local_files(self, **context):
        yield "/etc/systemd/system/gpioserver.service", service(
            "python3 -m gpioserver %s" % " ".join(generate_restpio_arguments()), **context
        )
        yield "/etc/systemd/system/lights.service", service(
            "python3 /home/pi/rdvhome/run.py run", **context
        )
