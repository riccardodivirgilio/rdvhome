...
from rdvhome.utils.colors import color_to_homekit, color_to_nanoleaf
...
class NanoleafControl(RemoteBase):
    ...
    def default_capabilities(self):
        return capabilities(on=True, hue=True, saturation=True, brightness=True,)
                            ...
                            "palette": tuple(
                                color_to_homekit(
                                    dict(
                                        brightness=1,
                                        hue=effect.hue,
                                        saturation=effect.saturation / (i + 1),
                                    )
                                )
                                for i in range(3)
                            ),
