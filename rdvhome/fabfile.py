...
class Device(object):
    def __init__(self, id, name=None, user=None, default_password="raspberry"):
        self.name = name
...
RASPBERRY = Device(id="rasp", name="rdvpi.local", user="pi", default_password="death4normals!")

DOORBELL = Device(
    id="doorbell", name="rdvdoorbell.local", user="pi", default_password="death4normals!"
)
...
def setup():
    ...
    sudo("apt-get update")
    sudo(
        "apt-get install ntpdate rsync python3 python3-rpi.gpio avahi-daemon avahi-discover libnss-mdns -y"
    )
